...
update_status ModuleCamera3D::Update()
{
	...
	if(App->input->GetMouseButton(SDL_BUTTON_RIGHT) == KEY_REPEAT)
	{
		...
		//LookAt(vec3(rotate(X, (float)(dx * 10), vec3(1, 0, 0)).x , rotate(X, (float)(dx * 10), vec3(1, 0, 0)).y, rotate(X, (float)(dx * 10), vec3(1, 0, 0)).z));
		//LookAt(vec3(rotate(Y, (float)(dy * 10), vec3(0, 1, 0)).x, rotate(Y, (float)(dy * 10), vec3(0, 1, 0)).y, rotate(Y, (float)(dy * 10), vec3(0, 1, 0)).z));
