...
bool PhysBody::Contains(int x, int y) const
{
	...
	return body->GetFixtureList()->GetShape()->TestPoint(body->GetTransform(), point);
}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	input.p1.Set(point1.x, point1.y);
	input.p2.Set(point2.x, point2.y);
	...
	if (hit) {
		...
		ret = sqrt(pow(x2-x1, 2) + pow(y2-y1, 2)) * output.fraction;
	}
